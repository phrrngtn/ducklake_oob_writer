...
The fix is **`recanonicalize(engine)`** — an **idempotent, in-place, set-based renumber** (one transaction, written with SQLAlchemy Core so a single implementation is dialect-portable across SQLite / PostgreSQL / DuckDB). It reorders the snapshots so `snapshot_id` order matches `snapshot_time` order, touching only the *moved* snapshots and the references to them. It's a cheap no-op when already ordered (a single inversion-count query), so you can call it unconditionally after a backfill. It leaves the rewrite- and hive-style dimension columns untouched (they carry no snapshot id), and the native reader doesn't depend on the writer-side `row_id_start`/allocation counters, so the renumber is the snapshot-id remap alone.
